Ducker:https://b11et3un53m.feishu.cn/wiki/MWQIw4Zvhil0I5ktPHwcoqZdnec

Linux安装教程：https://b11et3un53m.feishu.cn/wiki/FJAnwOhpIihMkLkOKQocdWZ7nUc

docker安装教程：https://b11et3un53m.feishu.cn/wiki/Rfocw7ctXij2RBkShcucLZbrn2d

# Docker

我们即将进入微服务阶段学习，微服务项目动辄就是几十台、上百台服务需要部署，有些大型项目甚至达到数万台服务。而**由于每台服务器的运行环境不同，你写好的安装流程、部署脚本并不一定在每个服务器都能正常运行**，经常会出错。这就给系统的部署运维带来了很多困难。
...
因为Docker会自动搜索并下载MySQL。注意：这里下载的不是安装包，而是**镜像。**镜像中不仅包含了MySQL本身，还包含了其运行所需要的环境、配置、系统级函数库。因此它在运行时就有自己独立的环境，就可以跨系统运行，也不需要手动再次配置环境了。这套独立运行的隔离环境我们称为**容器**。
...
[黑马视频](https://www.bilibili.com/video/BV1HP4118797?spm_id_from=333.788.videopod.episodes&vd_source=5063386180163934afae66c4e87325ac&p=5)
...
| docker logs (-f)   | 查看容器运行日志(持续跟踪)     | [docker logs](https://docs.docker.com/engine/reference/commandline/logs/) |
| docker exec  (-it) | 进入容器 (以可交互的终端)      | [docker exec](https://docs.docker.com/engine/reference/commandline/exec/) |
...
![image-20250120205332454](http://stofu80ry.sabkt.gdipper.com/picture/image-20250120205332454.png)
...
![image-20250120211232023](http://stofu80ry.sabkt.gdipper.com/picture/image-20250120211232023.png)
...
可以发现这个容器声明了一个本地目录，需要挂载数据卷，但是**数据卷未定义**。这就是**匿名卷**。
...
可以发现，数据卷的目录结构较深，如果我们去操作数据卷目录会不太方便。在很多情况下，我们会**直接将容器目录与宿主机指定目录挂载**。挂载语法与数据卷**类似**：
...
![img](http://stofu80ry.sabkt.gdipper.com/picture/1737379994157-7.png)
...
所以，就**有人提供了基础的系统加JDK环境**，我们在此基础上制作java镜像，就可以省去JDK的配置了：
...
Dockerfile中使用了openjdk:11.0-jre-buster，可以在外部直接下载后指定为镜像
...
所以，我们必须借助于docker的网络功能来解决这个问题（通过自定义一个新的网桥，容器可以在这个新的网桥中通过容器名进行访问，不用ip），官方文档：
...
![img](http://stofu80ry.sabkt.gdipper.com/picture/1737381336013-10.png)

我们要部署的就是其中的`hm-service`，其中的配置文件采用了多环境的方式：

![img](http://stofu80ry.sabkt.gdipper.com/picture/1737381336013-11.png)

其中的`application-dev.yaml`是部署到开发环境的配置，`application-local.yaml`是本地运行时的配置。

查看application.yaml，你会发现其中的JDBC地址并未写死，而是读取变量：

![img](http://stofu80ry.sabkt.gdipper.com/picture/1737381336013-12.png)

这两个变量在`application-dev.yaml`和`application-local.yaml`中并不相同：

![img](http://stofu80ry.sabkt.gdipper.com/picture/1737381336013-13.png)
...
| **类型** | **参数或指令**                                               | **说明**                    |
| :------- | :----------------------------------------------------------- | :-------------------------- |
| Options  | -f                                                           | 指定compose文件的路径和名称（不指定就是在当前目录） |
...
https://www.bilibili.com/video/BV1S142197x7?spm_id_from=333.788.player.switch&vd_source=5063386180163934afae66c4e87325ac&p=47
